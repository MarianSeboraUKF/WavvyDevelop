...
public class NowPlayingRepository {
    ...
    private static SharedPreferences sp(Context ctx) { return ctx.getSharedPreferences(PREFS, Context.MODE_PRIVATE); }
    public static void saveNowPlaying(Context ctx, int audioResId, int[] queueIds, int queueIndex) {
        ...
        sp(ctx).edit().putInt(KEY_AUDIO_ID, audioResId).putString(KEY_QUEUE_IDS, csv.toString()).putInt(KEY_QUEUE_INDEX, queueIndex).apply();
    }
    ...
    public static void savePosition(Context ctx, long positionMs) {
        sp(ctx).edit().putLong(KEY_POSITION, positionMs).apply();
    }
